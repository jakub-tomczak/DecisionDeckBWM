# usage:
# R --slave --vanilla --file=BWMCLI_XMCDAv2.R --args "[inDirectory]" "[outDirectory]"
...
#inDirectory <- "/path/to/BWM/tests/in1.v2"
#outDirectory <- "/path/to/BWM/tests/out_tmp/"

# filenames

criteriaFile <- "criteria.xml"
bestPreferencesOfCriteriaFile <- "bestPreferencesOfCriteria.xml"
worstPreferencesOfCriteriaFile <- "worstPreferencesOfCriteria.xml"
...
loadXMCDAv3(xmcdaData, inDirectory, bestPreferencesOfCriteriaFile, mandatory = TRUE, xmcdaMessages, "criteriaValues")
loadXMCDAv3(xmcdaData, inDirectory, worstPreferencesOfCriteriaFile, mandatory = TRUE, xmcdaMessages, "criteriaValues")
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert BWM results into XMCDA")
}
